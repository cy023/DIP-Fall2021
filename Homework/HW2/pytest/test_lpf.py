...
spectrum = 10 * np.log(np.abs(y))
img = Image.fromarray(np.uint8(np.round_(spectrum)), 'L')
img.save('./Image/_hey.bmp')

lpf_norm = np.zeros(y.shape, np.uint8)

for i in range(spectrum.shape[0]):
    for j in range(spectrum.shape[1]):
        if np.sqrt((i-128)**2 + (j-128)**2) > 50:
            y[i][j] = 0
            lpf_norm[i][j] = 0
        else:
            lpf_norm[i][j] = 10 * np.log(np.abs(y[i][j]))
            
img = Image.fromarray(np.uint8(np.round_(lpf_norm)), 'L')
img.save('./Image/_new.bmp')

x = np.fft.ifft2(y)
x = pretf(x.real)

img = Image.fromarray(np.uint8(x), 'L')
img.save('./Image/_hand.bmp')
